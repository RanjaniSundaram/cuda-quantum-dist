...
public:
  ...
  void DynamicProgram(std::vector<std::vector<int>> distMat) {
    ...
    os << "Dynamic Programming "
       << "\n";
    std::vector<std::vector<int>> SolutionCost(numGates,
                                               std::vector<int>(numGates, 0));
    // std::vector<std::vector<std::vector< std::tuple<int,int> >>>
    // SolutionIndices;
    std::vector<std::vector<std::vector<std::tuple<int, int>>>> SolutionIndices(
        numGates, std::vector<std::vector<std::tuple<int, int>>>(
                      numGates, std::vector<std::tuple<int, int>>(numGates)));
    std::vector<std::vector<std::vector<int>>> SubcircuitSolutions(
        numGates,
        std::vector<std::vector<int>>(numGates, std::vector<int>(numGates, 0)));
    std::vector<std::vector<int>> SubcircuitCost(numGates,
                                                 std::vector<int>(numGates, 0));
    int MinVal = INT_MAX;
    int MinIndex = 0;
    int CurrVal;
      ...
      for (int j = i + 1; j < numGates; j++) {
        ...
        for (int k = 0; k < i; k++) {
          CurrVal = SolutionCost[k][i] + SubcircuitCost[i][j] +
                    StitchingCost(SubcircuitSolutions[k][i],
                                  SubcircuitSolutions[i][j], distMat);
          if (MinVal >= CurrVal) {
            MinVal = CurrVal;
            MinIndex = k;
          }
...
private:
  ...
  int StitchingCost(std::vector<int> soln1, std::vector<int> soln2,
  ...
  // A function that returns a flow matrix corresponding to the sub-circuit
  // contained
  // between the gate indices i and j
  std::vector<std::vector<int>> FlowMatrix(int i, int j) {
    // llvm::raw_ostream &os = llvm::errs();
    // os << "Flow matrix for subcircuit " <<"\n";
    mlir::SmallVector<std::tuple<Qubit, Qubit>> SubCircuit(
        Circuit.gates.begin() + i, Circuit.gates.begin() + j);
    // os << "Flow matrix  "<<numQubits <<"\n";
    std::vector<std::vector<int>> flow(numQubits,
                                       std::vector<int>(numQubits, 0));
